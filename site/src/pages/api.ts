export function ApiPage(): string {
  ...
  const exportRows = exports
    .map(
      ([name, type, desc]) =>
        `<tr><td><code>${name}</code></td><td>${type}</td><td>${desc}</td></tr>`,
    )
    .join("");
...
const pretty = formatJson('{"a":1,"b":2}', "  ");</code></pre>
    </div>
    <div class="section">
...
const errors = validator.validate({ name: "Alice", age: 30 }, schema);</code></pre>
    </div>
    <div class="section">
      <h2>Exports</h2>
      <table>
